...
🚚 SELLERS NOW RUN THEIR OWN ORDERS, RETURNS AND PAYMENTS — THE AMAZON SELLER CENTRAL WAY. Manage Orders replaces the read-only order list: every order shows a Ship-by date, and the seller confirms it, marks it packed, confirms the shipment with the carrier and tracking ID, marks it delivered, or cancels it with a reason — each step emails the customer, and the customer's Track My Order page (now order ID plus mobile number, read live from the sheet instead of a demo timeline) shows the carrier and tracking number the moment they are entered. Packing slip and shipping label print from the order card. Manage Returns: a customer requests a return from Track My Order with Amazon's reason list and photos; the seller authorizes, declines with a reason, or marks the item received; IBI records the refund, and it lands as an adjustment on the seller's statement. Payments: settlement periods — the Monday after the 7-day return window — with item total, 5% commission, 18% GST on the commission, refunds and net, printable and downloadable as a spreadsheet. Registration verifies the email with a 6-digit code before the PIN can ever be sent to it; Forgot PIN emails a fresh PIN to the registered address; Change PIN sits in Profile. Seller Central opens with Complete-your-setup — agreement, identity, bank, KYC, first listing, first approval — and an Account Health strip with late-shipment, cancellation and return rates against Amazon's targets. Bulk Upload gains an IBI template download and a processing report. Also fixed: customer orders were never reaching the Orders sheet (the save demanded a CEO token a shopper does not have); orders now save for everyone, with prices re-checked against the catalogue.

## v15.0

🪪 SELLER ONBOARDING NOW MEETS THE MARKETPLACE STANDARD: AGREEMENT, BANK, ADDRESS AND KYC. Becoming a seller is a four-step form, the same sequence Amazon, Flipkart and Meesho use. Step one takes the business details with PAN made mandatory and GSTIN optional for the unregistered; step two the registered address with state and pincode, and the pickup address; step three the payout bank account, typed twice and checked against the IFSC format; step four the KYC documents — PAN card, GST certificate, cancelled cheque and address proof — uploaded to a private folder that is never publicly shared, and a tick to accept the new Seller Agreement & Policies, a public page linked from the footer that sets out the 5% commission, weekly payouts, shipping and return terms, prohibited items, and the grievance officer the E-Commerce Rules require. The form cross-checks the PAN against the PAN embedded in the GSTIN and the GSTIN's state code against the state chosen, so a mistyped number is caught before it reaches us. The Profile tab now shows all of this — the bank account masked to its last four digits, each document's status, and the KYC verdict — and lets the seller correct it; changing the bank account or a document puts KYC back to Pending for re-verification. The admin panel shows every detail, the documents, and a KYC status the team sets to Verified or Rejected with a note the seller sees. Backend: eighteen new columns on the Sellers sheet, added automatically, and a profile-update call that needs the seller's own PIN, so a seller can finally save their profile without a CEO token.

## v13.0

💍 THE JEWELLERY IS NOW CALLED FASHION JEWELLERY, SITS TOGETHER, AND COUNTS ITSELF FROM THE JEWELLERY REGISTER. Three things, all about the same four listings. First the name: the department was headed "Imitation Jewellery" and now reads "Fashion Jewellery" everywhere it appears — the shop heading, the department bar, the category menus and the admin panel. Second, they are together at last: only one of the four was appearing under that heading, because the shop decides which family a product belongs to by reading its title, and these titles say "Crystal Stone Pendant" — so three chokers were being filed under Stone Products, in a different part of the page from the one they belonged to. A listing's own category is now trusted ahead of the wording of its title, so all four sit in one place, and any jewellery listed in future joins them automatically. Third, and the reason they all read Out of Stock: fashion jewellery is not counted in the Stock Availability app at all — it has its own register, the IBI Fashion Jewellery app on the office laptop — so the nightly job could find no count for any of them and the rule that nothing may be sold unless it can be shipped took every one off sale, while about thirty-four pieces sat on the shelf. The shop now asks the jewellery register for those listings and the Stock Availability app for everything else, and the jewellery app tells the shop the moment a piece is added or sold, so the two agree within the minute. Colour is treated as part of the product's identity here, because in this department one piece differs from the next by nothing else: a chain recorded as Red & Black can no longer hand its count to the plain Black variant, and where two variants of one listing both point at the same shelf neither is allowed to claim it — that is a warning to fix the listing, not a number to guess at. If the laptop is off, the jewellery listings are left exactly as they are rather than emptied.
